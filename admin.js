...
// Event Delegation for Pending Submissions
submissionsContainer.addEventListener('click', async (e) => {
    ...
    if (e.target.classList.contains('approve-btn')) {
        ...
        try {
            ...
            await updateDoc(movieDocRef, { status: 'Approved', showDate, trailerLink, posterURL });
...
// Drag and Drop Listeners for Pending Submissions
...
// Event Delegation for Approved Movies
approvedMoviesContainer.addEventListener('click', async (e) => {
    ...
    if (e.target.classList.contains('edit-btn')) {
        const querySnapshot = await getDocs(query(collection(db, 'movies'), where(doc(db, 'movies').id, '==', movieId)));
        const movieData = { id: movieId, ...doc.data() };
    ...
    if (e.target.classList.contains('cancel-btn')) {
        const querySnapshot = await getDocs(query(collection(db, 'movies'), where(doc(db, 'movies').id, '==', movieId)));
        const movieData = { id: movieId, ...doc.data() };
        card.innerHTML = createApprovedCardView(movieData);
    }
});
